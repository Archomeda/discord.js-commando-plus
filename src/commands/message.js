...
class CommandMessage {
    ...
    usage(argString, prefix, user = this.client.user) {
        if (typeof prefix === 'undefined') {
            if (this.message.guild) {
                prefix = this.message.guild.commandPrefix;
            } else {
     ...
     * @return {string|string[]} The parsed arguments.
    ...
    parseArgs() {
        switch (this.command.argsType) {
            ...
            case 'multiple':
                return this.constructor.parseArgs(this.argString,
                    this.command.argsCount, this.command.argsSingleQuotes);
            default:
    ...
    async run() { // eslint-disable-line complexity
        ...
        try {
            ...
            if (!(retVal instanceof discord.Message || retVal instanceof Array ||
        ...
        switch (type) {
            ...
            case 'reply':
                ...
                if (options && options.split && !options.split.prepend) {
                    options.split.prepend = `${this.message.author}, `;
                }
                return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
            case 'direct':
                if (!shouldEdit) {
    ...
    editResponse(response, { type, content, options }) {
        ...
        if (options && options.split) {
            content = discord.splitMessage(content, options.split);
        }

        let prepend = '';
        if (type === 'reply') {
            prepend = `${this.message.author}, `;
        }
     ...
     * @private
     */
    editCurrentResponse(id, { type, content, options }) {
     ...
     * @param {?Message|Message[]|Array<Message[]>} responses - Response(s) to the message
     * @return {void}
     * @private
     */
    finalize(responses) {
     ...
     * @return {Promise<MessageReaction>} The reaction.
     ...
     * @return {Promise<?Webhook>} The webhook.
